...
class MainHandler(BaseHandler):
    ...
    def get(self):
        ...
        # (by default) for world cup.
        feed = feedparser.parse("http://pipes.yahoo.com/pipes/pipe.run?_id=1nWYbWm82xGjQylL00qv4w&_render=rss&textinput1=world cup" )
        ...
        wordcloud = self.gather_words(feed)
        wordcloud = wordcloud[0:50]

        # now we place the feed in context and provide the default search term
        context = {"feed" : feed, "search" : "world cup", "wordcloud":wordcloud}
    ...
    def gather_words(self, feed):
        ...
        for key, value in all_words.iteritems():
            if not key in common:
                wordcloud = wordcloud + [{"text":key, "size":value*20}]
        return wordcloud
